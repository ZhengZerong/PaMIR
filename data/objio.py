...
def save_obj_data(filename, model):
    ...
    with open(filename, 'w') as fp:
        if 'mtllib' in model and model['mtllib'] is not None:
            fp.write(model['mtllib'])
        if 'usemtl' in model and model['usemtl'] is not None:
            fp.write(model['usemtl'])
