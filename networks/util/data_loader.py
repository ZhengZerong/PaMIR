...
class ManualSampler(Sampler):
    def __init__(self, data_source, checkpoint, dataset_perm):
        self.data_source = data_source
        if checkpoint is not None and checkpoint['dataset_perm'] is not None:
            self.dataset_perm = checkpoint['dataset_perm']
            self.perm = self.dataset_perm[checkpoint['batch_size'] * checkpoint['batch_idx']:]
        else:
            self.dataset_perm = dataset_perm
            self.perm = self.dataset_perm
...
class CheckpointDataLoader(DataLoader):
    """
    Extends torch.utils.data.DataLoader to handle resuming training
    from an arbitrary point within an epoch.
    """
    def __init__(self, dataset, checkpoint=None, batch_size=1, dataset_perm=None,
                 shuffle=False, num_workers=0, pin_memory=False, drop_last=True,
                 timeout=0, worker_init_fn=None):

        if dataset_perm is not None:
            sampler = ManualSampler(dataset, checkpoint, dataset_perm)
        elif shuffle:
